...
class AddConsoleCommandPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($commandServices as $id => $tags) {
            ...
            if (!is_subclass_of($class, 'Symfony2\\Component\\Console\\Command\\Command')) {
                if (!class_exists($class, false)) {
                    throw new \InvalidArgumentException(sprintf('Class "%s" used for service "%s" cannot be found.', $class, $id));
                }

                throw new \InvalidArgumentException(sprintf('The service "%s" tagged "console.command" must be a subclass of "Symfony2\\Component\\Console\\Command\\Command".', $id));
            }
