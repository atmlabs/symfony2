...
use Symfony2\Component\Security\Core\Exception\AccountExpiredException;
use Symfony2\Component\Security\Core\Exception\BadCredentialsException;
use Symfony2\Component\Security\Core\Exception\CredentialsExpiredException;
...
class UserAuthenticationProviderTest extends TestCase
{
    ...
    public function testAuthenticateWhenPreChecksFails()
    {
        ...
        $provider->expects($this->once())
                 ->method('retrieveUser')
                 ->will($this->returnValue($this->getMockBuilder('Symfony2\Component\Security\Core\User\UserInterface')->getMock()))
        ;
    ...
    public function testAuthenticate()
    {
        ...
        $this->assertInstanceOf('Symfony2\Component\Security\Core\Authentication\Token\UsernamePasswordToken', $authToken);
    ...
    public function testAuthenticateWithPreservingRoleSwitchUserRole()
    {
        ...
        $switchUserRole = new SwitchUserRole('foo', $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Token\TokenInterface')->getMock());
        $token->expects($this->once())
              ->method('getRoles')
    ...
    {
        $mock = $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Token\UsernamePasswordToken')->setMethods(array('getCredentials', 'getProviderKey', 'getRoles'))->disableOriginalConstructor()->getMock();
        $mock
            ->expects($this->any())
    ...
    protected function getProvider($userChecker = false, $hide = true)
    {
        ...
        return $this->getMockForAbstractClass('Symfony2\Component\Security\Core\Authentication\Provider\UserAuthenticationProvider', array($userChecker, 'key', $hide));
    }
}
