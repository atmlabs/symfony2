...
use Symfony2\Component\Security\Core\Authentication\AuthenticationProviderManager;
...
class AuthenticationProviderManagerTest extends TestCase
{
    ...
    public function testAuthenticateWhenNoProviderSupportsToken()
    {
        ...
        try {
            $manager->authenticate($token = $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Token\TokenInterface')->getMock());
            $this->fail();
    ...
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Symfony2\Component\Security\Core\Exception\AccountStatusException'),
        ));
    ...
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Symfony2\Component\Security\Core\Exception\AuthenticationException'),
        ));
    ...
    public function testAuthenticateWhenOneReturnsAuthenticationExceptionButNotAll()
    {
        $manager = new AuthenticationProviderManager(array(
            $this->getAuthenticationProvider(true, null, 'Symfony2\Component\Security\Core\Exception\AuthenticationException'),
            $this->getAuthenticationProvider(true, $expected = $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Token\TokenInterface')->getMock()),
        ));

        $token = $manager->authenticate($this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Token\TokenInterface')->getMock());
        $this->assertSame($expected, $token);
    }

    public function testAuthenticateReturnsTokenOfTheFirstMatchingProvider()
    {
        $second = $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Provider\AuthenticationProviderInterface')->getMock();
        $second
            ->expects($this->never())
            ->method('supports')
        ;
    ...
    public function testAuthenticateDispatchesAuthenticationFailureEvent()
    {
        ...
        $provider = $this->getMockBuilder('Symfony2\Component\Security\Core\Authentication\Provider\AuthenticationProviderInterface')->getMock();
