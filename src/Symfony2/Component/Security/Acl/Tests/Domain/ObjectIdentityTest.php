...
namespace Symfony2\Component\Security\Acl\Tests\Domain
{
    ...
    class ObjectIdentityTest extends \PHPUnit_Framework_TestCase
    {
        ...
        {
            $id = new ObjectIdentity('fooid', 'Acme\DemoBundle\Proxy\__CG__\Symfony2\Component\Security\Acl\Tests\Domain\TestDomainObject');

            $this->assertEquals('fooid', $id->getIdentifier());
            $this->assertEquals('Acme\DemoBundle\Proxy\__CG__\Symfony2\Component\Security\Acl\Tests\Domain\TestDomainObject', $id->getType());
        }

        public function testFromDomainObjectPrefersInterfaceOverGetId()
        {
            $domainObject = $this->getMock('Symfony2\Component\Security\Acl\Model\DomainObjectInterface');
            $domainObject
                ->expects($this->once())
        ...
        {
            $id = ObjectIdentity::fromDomainObject(new \Acme\DemoBundle\Proxy\__CG__\Symfony2\Component\Security\Acl\Tests\Domain\TestDomainObject());
            $this->assertEquals('getId()', $id->getIdentifier());
            $this->assertEquals('Symfony2\Component\Security\Acl\Tests\Domain\TestDomainObject', $id->getType());
        }
