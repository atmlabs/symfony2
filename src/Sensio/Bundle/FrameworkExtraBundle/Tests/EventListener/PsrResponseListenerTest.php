...
use Symfony2\Bridge\PsrHttpMessage\Factory\HttpFoundationFactory;
use Symfony2\Bridge\PsrHttpMessage\Tests\Fixtures\Response;
...
class PsrResponseListenerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        if (!class_exists('Symfony2\Bridge\PsrHttpMessage\HttpFoundationFactoryInterface')) {
    ...
    public function testConvertsControllerResult()
    {
        ...
        $event->expects($this->once())->method('setResponse')->with($this->isInstanceOf('Symfony2\Component\HttpFoundation\Response'));
    ...
    {
        $event = $this->getMock('Symfony2\Component\HttpKernel\Event\GetResponseForControllerResultEvent', array(), array(), '', null);
        $event
            ->expects($this->any())
